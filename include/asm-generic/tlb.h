...
static inline void tlb_unshare_pmd_ptdesc(struct mmu_gather *tlb, struct page *page,
					  unsigned long addr)
{
	/*
	 ...
	 */
	VM_WARN_ON_ONCE(!atomic_read(&page->pt_share_count));
	 ...
	 * a page table.
	 */
	if (!atomic_read(&page->pt_share_count))
